...
SITS="/ebio/abt6_projects9/pathodopsis_microbiomes/pathodopsis_git/code/ITS"

input1=/ebio/abt6_projects9/pathodopsis_microbiomes/data/raw_reads/ITS/run145_ITS_1_2019

input2=/ebio/abt6_projects9/pathodopsis_microbiomes/data/raw_reads/ITS/run166_ITS_9_2019
...
############
mkdir $output_direc/run145_ITS_1_2019

mkdir $output_direc/run166_ITS_9_2019

mkdir $output_direc/run145_ITS_1_2019/demult_python

mkdir $output_direc/run166_ITS_9_2019/demult_python
############

#Now we demultiplex the two separate runs. This first script is a for loop that then executes a shell script that calls a python demultiplexing script.
$SITS/demultiplex_ITS_non_flashed.sh $input1 $output_direc/run145_ITS_1_2019

$SITS/demultiplex_ITS_non_flashed.sh $input2 $output_direc/run166_ITS_9_2019
...
ls $output_direc/run145_ITS_1_2019/demult_python/ > run145.txt

ls $output_direc/run166_ITS_9_2019/demult_python/ > run166.txt

#Find the union of the barcodes across runs
cat run145.txt run166.txt > all.txt
cat all.txt | sort | uniq -d > union.txt
...
for file1 in `cat union.txt`; do
    # cat them together
    echo $file1
    cat $output_direc/run145_ITS_1_2019/demult_python/$file1 > $output_direc/all.$file1
    cat $output_direc/run166_ITS_9_2019/demult_python/$file1 >> $output_direc/all.$file1
done
...
$SITS/clip_barcodes_after_python_ITS.sh $output_direc/ $demultiplexing_file
