...
option_list <- list(
  ...
  make_option(c("--f_match"), type="character", default="_R1.*fastq.*",
              help="Regular expression to match in forward reads' filenames (default: \"_R1_.*fastq.*\").",
              metavar="PATTERN"),

  make_option(c("--r_match"), type="character", default="_R2.*fastq.*",
              help="Regular expression to match in reverse reads' filenames (default: \"_R2_.*fastq.*\").",
              metavar="PATTERN"),

  make_option(c("--sample_delim"), type="character", default="_16S",
